...
export function generateMetadata({ params }: { params: PageParams }): Metadata {
  ...
  return {
    title: `Weather in ${destination.name}`,
    description: `Current weather, 7-day forecast and the best time to visit ${destination.name}, ${destination.state}.`
  };
}

export default async function DestinationWeatherPage({ params }: { params: PageParams }) {
    ...
    <>
      <Breadcrumbs
        items={[
          ...
          {
            label: dict.common.nav.weather,
            href: `/${locale}/india/${destination.stateSlug}/${destination.slug}/weather`
          }
        ]}
      />
      <SubPageHeader
        ...
        locale={locale}
        title={`${dict.destination.sections.weather} — ${destination.name}`}
      />
      <WeatherSection destination={destination} dict={dict} id="weather-full" />
    </>
  );
}
