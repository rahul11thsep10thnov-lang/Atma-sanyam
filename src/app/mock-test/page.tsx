...
export default async function MockTestPage({
  ...
  return (
    <div className="container-page py-8">
      <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900">State Police Mock Tests</h1>
      <p className="mt-1 text-sm text-gray-600">
        Full mock, subject test, state GK test — timer aur result analysis ke saath.
      </p>

      <div className="mt-5 flex flex-wrap gap-2">
        <Link href="/mock-test" className={`rounded-full px-3 py-1.5 text-xs font-semibold ${!sp.state ? "bg-brand-navy text-white" : "bg-gray-100 text-gray-600"}`}>
          Sabhi States
        </Link>
        {STATES.map((s) => (
          <Link
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((m) => (
          <Link key={m.id} href={`/mock-test/${m.id}`} className="card p-4 hover:shadow-md transition-shadow">
