      ...
      <div className="container-page flex h-16 items-center justify-between gap-4">
        ...
        <nav className="hidden lg:flex items-center gap-1 overflow-x-auto text-sm font-medium">
          {MAIN_NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="whitespace-nowrap rounded-md px-3 py-2 text-gray-700 hover:bg-gray-100 hover:text-brand-navy"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            href="/search"
            aria-label="Search"
            className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--card-border)] text-gray-600 hover:bg-gray-100"
          >
            <Search size={18} />
          </Link>
          <Link
            href="/login"
            className="btn-primary hidden sm:inline-flex items-center px-4 py-2 text-sm"
          >
            Login
          </Link>
        </div>
